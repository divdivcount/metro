		...
		if($this->quTable == 'interest'){
			if($mb_id != 'null' && $om_id == 'null'){
				...
				if($mb_id != 'null' && $om_id == 'null'){
					...
					$mb_id = null;
					$sql = "select om.om_nickname,p.pr_id,p.pr_title,p.pr_status,p.pr_price, (select count(i.in_hit) from interest i where i.pr_id = p.pr_id) as i_count, pi.pr_img, l.l_name, p.pr_station,(select count(md.pr_id) from member_declaration md where md.pr_id = p.pr_id) as rep_count from product p left outer join product_img pi ON p.pr_img_id = pi.pr_img_id left outer join line l ON p.l_id = l.l_id left outer join oauth_member om ON p.om_id = om.om_id where p.om_id = :om_id and p.pr_img_id = pi.pr_img_id and pi.main_check = 'y' and  p.mb_id is :mb_id and pr_block = 1 order by $this->quTableId desc limit :start, :viewLen";
				}elseif($mb_id == 'null' && $om_id == 'null'){
						if(empty($s_value) == true){
