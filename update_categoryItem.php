<?php
  ...
  $sql = "select p.pr_id,(select pr_img from product_img pi where pi.pr_img_id = p.pr_img_id and pi.main_check = 'y') as pr_img, p.pr_title, l_id ,p.pr_station, p.pr_price, (select count(i.in_hit) from interest i where p.pr_id = i.pr_id )as in_hit, p.ca_name from product p where p.ca_name ='$category' and pr_status='판매중' and pr_block = 1 order by in_hit desc";
  $result = mysqli_query($conn, $sql);

  try {

    while ($row = mysqli_fetch_assoc($result)) {
      echo '<a href="searchProduct_detail.php?id='.$row['pr_id'].'&title='.$row['pr_title'].'line='.$row['l_id'].'">'.'<div class="productInfo_box">';
